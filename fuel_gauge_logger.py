...
filename = './fuel_gauge.csv'


def write_to_file(ts, batt_voltage, batt_charge, batt_charge_rate):
    
    file = open(filename, 'a')
    file.write(f"{ts}, {batt_voltage}, {batt_charge}, {batt_charge_rate}")
    file.close()
...
while True: 
    ...
    write_to_file(datetime.datetime.now(), max17.cell_voltage, max17.cell_percent, max17.charge_rate)

    print(f"Battery voltage: {max17.cell_voltage:.2f} Volts")
    print(f"Battery state  : {max17.cell_percent:.1f} %")
    print(f"Battery (dis)charge rate: {max17.charge_rate:.2f}} %/hr")
